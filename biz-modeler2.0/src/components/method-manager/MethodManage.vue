  ...
  <div>
		<el-form :inline="true"
			...
			<el-form-item label="方法分类" style="margin-left: 20px">
				<el-input v-model="pageData.edmm_type_name" size="small"
				          placeholder=""  icon="plus"  @focus="OpenTree" @click="OpenTree" @blur="blurType" readonly></el-input>
				<input type="hidden"
				       v-model="pageData.edmmType">
			</el-form-item>
			<el-form-item label="程序类型">
        <el-select v-model="pageData.edmmProgramType" size="small" filterable>
                  <el-option :key="''" :label="'全部'"
					           :value="''"></el-option>
					<el-option v-for="options in pageData.edmm_program_type" :key="options.id" :label="options.codeName"
			...
			<el-form-item label="方法状态">
				<el-select v-model="pageData.edmmStatus" size="small" filterable>
					<el-option v-for="options in pageData.edmm_status" :key="options.id" :label="options.label"
...
<!--
    ...
    :height="$store.state.windowStore.windowHeight - 185">
        ...
        <div v-else>否</div>
			...
			<el-table-column label="平台方法" align='center' width="80">
      <template scope="scope">
			...
			<el-table-column label="操作"
				...
				<template scope="scope">
					<el-button size="small"
					           ...
					           type="operate" icon="rx-bianji2" title="编辑"></el-button>
				</template>
			</el-table-column>
		</el-table>
  <div class="mright">
